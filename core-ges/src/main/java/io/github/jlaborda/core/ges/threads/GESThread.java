...
public abstract class GESThread implements Runnable{
    ...
    protected Set<Edge> edgesCandidates;
...
//        }
    ...
    protected void pdag(Graph graph) {
        ...
        rules.setMeekPreventCycles(this.aggressivelyPreventCycles);
...
//        Graph dag = SearchGraphUtils.dagFromPattern(graph);
    ...
    public double scoreDag(Graph graph) {
        ...
        for (Node node : getVariables()) {
            ...
            final double nodeScore = problem.evaluate(hashIndices.get(node), parentIndices);
            _score += nodeScore;
        }
        return _score;
    }
...
*/
    ...
    public static double localBdeuScore(int nNode, int[] nParents, Problem problem) {
        ...
        return problem.evaluate(nNode, nParents);
    }
    ...
    public void setMaxNumEdges(int maxNumEdges) {
        if (maxNumEdges < -1)
            throw new IllegalArgumentException("maxNumEdges must be -1 (no limit) or a positive integer");
     ...
     */
    public void setMeekPreventCycles(boolean aggressivelyPreventCycles) {
